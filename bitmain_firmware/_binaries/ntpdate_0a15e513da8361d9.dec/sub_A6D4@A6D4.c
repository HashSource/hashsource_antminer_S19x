char *__fastcall sub_A6D4(char *result, char *s)
{
  char *v2; // r9
  int v4; // r5
  int v5; // r8
  ...
  v11 = 0;
  v4 = progname;
  if ( progname != dword_33310 )
  {
    dword_33310 = progname;
    result = strrchr((const char *)progname, 47);
    if ( result )
      dword_33314 = (int)++result;
    else
      dword_33314 = v4;
  }
  v5 = msyslog_term;
...
LABEL_11:
  ...
  if ( v5 )
  {
    ...
    if ( msyslog_term_pid )
      _fprintf_chk(v9, 1, "%s[%d]: ", (const char *)dword_33314, v7);
  ...
  if ( v6 )
  {
    ...
    _fprintf_chk(syslog_file, 1, "%s[%d]: %s%s", (const char *)dword_33314, v7, s, v8);
