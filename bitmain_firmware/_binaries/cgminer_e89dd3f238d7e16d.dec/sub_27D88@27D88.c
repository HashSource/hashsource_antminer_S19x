int __fastcall sub_27D88(const char **a1, int a2, char a3)
{
  uint32_t v3; // r0
  size_t v5; // r3
  ...
  if ( netlong == -1 )
  {
    ...
    memset(v11, 0, 0x20u);
    v11[0].ai_family = 2;
    if ( !getaddrinfo(a1[150], 0, v11, &pai) )
    {
      ai_addr = pai->ai_addr;
  ...
  if ( a3 != 1 )
  {
    if ( netlong == -1 )
    {
      ...
      {
        snprintf((char *)v11, 0x800u, "Invalid IP address specified for socks4 proxy: %s", a1[150]);
        sub_1DB6C(4, (const char *)v11, 0);
      }
      return 0;
    }
    v14 = HIBYTE(netlong);
  ...
  else
  {
    ...
    if ( (int)n > 255 )
      n = 255;
    sub_2AF08(&v19, a1[150], n, "util.c", "socks4_negotiate", 2981);
    n += 16;
    v5 = n++;
    buf[v5] = 0;
    send(a2, buf, n, 0);
  }
  if ( !sub_270FC(a2) && sub_270FC(a2) == 90 )
  {
    for ( i = 0; i <= 5; ++i )
      sub_270FC(a2);
    return 1;
  ...
  else
  {
    if ( byte_90DC0 || byte_865D1 || dword_857E4 > 3 )
    {
      snprintf((char *)v11, 0x800u, "Bad response from %s:%s SOCKS4 server", a1[151], a1[152]);
      sub_1DB6C(4, (const char *)v11, 0);
    }
    return 0;
  }
