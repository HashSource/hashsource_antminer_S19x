int __fastcall sub_8D28C(char *a1, int a2)
{
  char *v3; // r0
  int *v4; // r0
  size_t v5; // r0
  ...
  while ( 1 )
  {
    ...
    if ( *(s - 1) != 41 )
    {
      sub_8CD10(2, "src/zc_util.c", 124, "in string[%s] can't find match )", (const char *)dest);
      return -1;
    }
    v3 = getenv(v11);
    ...
    {
      v4 = _errno_location();
      sub_8CD10(2, "src/zc_util.c", 131, "snprintf fail, errno[%d], evn_value_len[%d]", *v4, n);
      return -1;
    }
    v16 += (_BYTE *)dest - s + n;
    if ( v16 > a2 - 1 )
  ...
  sub_8CD10(2, "src/zc_util.c", 137, "repalce env_value[%s] cause overlap", v10);
  return -1;
}
